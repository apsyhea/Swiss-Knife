...
async def blocklist(message: types.Message) -> None:
    ...
    for data in json_data.values():
        ...
        if any(re.search(query, url) for url in urls) or query in ips:
            is_blocked = True
            await message.reply(f"🌐 <b>{query} IP address blocked on the territory of Ukraine</b>", parse_mode="HTML")
            break

    if not is_blocked:
        await message.reply(f"🌐 <b>{query} IP address is not blocked on the territory of Ukraine</b>", parse_mode="HTML")
